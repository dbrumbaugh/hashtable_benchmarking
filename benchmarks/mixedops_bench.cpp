...
HashTable<uint32_t, int> *initialize_table(size_t n, size_t maxitr)
{
    auto table = new HashTable<uint32_t, int>(&testhash0, &testhash1, maxitr);
...
{
    if (argc < 3) {
        cerr << "useage: mixedops <maxitr> <write-mix>";
        exit(1);
    }

    size_t maxitr = stoi(argv[1]);
    double write_mix = stod(argv[2]);

    auto test = initialize_table(1000000, maxitr);
